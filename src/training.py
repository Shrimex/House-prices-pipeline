...
def run_training_pipeline(config):
    ...
    best_cv_RMSE       = float(results_df.iloc[0]["RMSE_mean"])
    best_cv_r_square = float(results_df.iloc[0]["r_square_mean"])

    print("Результаты кросс-валидации (дефолтные параметры):")
    print(results_df.to_string(index=False))
    print(f"\nЛучшая модель: {best_model_name} (RMSE={best_cv_RMSE:.4f})")
    ...
    for model_name, model in models.items():
        if model_name == "dummy_regressor" or model_name not in param_grids:
            continue
        ...
        grid_results.append({
            ...
            "best_RMSE": abs(search.best_score_),
            "best_params": search.best_params_,
            "best_estimator": search.best_estimator_,
        })
        print(f"  RMSE={abs(search.best_score_):.4f} params={search.best_params_}")

    grid_results_df = (
        pd.DataFrame(grid_results)
        .sort_values("best_RMSE", ascending=True)
    ...
    best_search_RMSE   = best["best_RMSE"]

    print("\nРезультаты после GridSearchCV:")
    print(grid_results_df[["model_name", "best_RMSE"]].to_string(index=False))

    test_predictions = np.expm1(final_model.predict(X_test_processed))
        ...
        save_json(
            {
                ...
                "best_cv_r_square":     best_cv_r_square,
                "selected_final_model": selected_model_name,
                "best_params":          best_params,
                "best_search_RMSE":     best_search_RMSE,
            },
    ...
    print(f"RMSE лучшей конфигурации: {best_search_RMSE:.4f}")
